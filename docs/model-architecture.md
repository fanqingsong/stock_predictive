...
> 旧版 `v5-scale`（无大盘相对特征）已废弃，需按本版重新训练。
...
flowchart TB
  ...
  subgraph models [独立模型 · 输入形状 = 序列长度 × 特征数]
    DayBars --> FeatD["日特征 15 维"]
    WeekBars --> FeatW["周特征 15 维"]
    MonthBars --> FeatM["月特征 15 维"]
    FeatD --> NetD["日网络 LSTM 2×64<br/>序列长度 T=10 · 特征 F=15<br/>in: B×10×15"]
    FeatW --> NetW["周网络 LSTM 2×64<br/>序列长度 T=12 · 特征 F=15<br/>in: B×12×15"]
    FeatM --> NetM["月网络 LSTM 1×32<br/>序列长度 T=6 · 特征 F=15<br/>in: B×6×15"]
  end
...
|------|----------|------------|----------|----------|------|----------|
| **日** | 交易日 | **10** | **15** | `(B, 10, 15)` | LSTM×2, h=64, dropout=0.3 | 下一 **交易日** 涨/跌 |
| **周** | 自然周 | **12** | **15** | `(B, 12, 15)` | LSTM×2, h=64, dropout=0.3 | 下一 **自然周** 涨/跌 |
| **月** | 自然月 | **6** | **15** | `(B, 6, 15)` | LSTM×1, h=32, dropout=0.4 | 下一 **自然月** 涨/跌 |
...
三套模型共用同一类 `StockLSTM`，仅 `T / F / 层数 / hidden` 不同。下图以 **日模型（T=10, F=15, 2×64）** 为模板，标出时间流向与层级流向。

### 1.1 总拓扑：时间从左到右 · 层从底到顶

```mermaid
flowchart TB
  subgraph L0 ["① 底层输入 · 特征维 F=15"]
    direction LR
...
2. **纵向（底层→高层）**：每个时间步先喂 **15 维特征** → 第 1 层隐状态 → 第 2 层隐状态 →（仅末步）Dropout → Linear → **涨/跌**。  
...
flowchart LR
  subgraph inSide [输入]
    B1["最近 T=10 个交易日"]
    B1 --> F1["每日 F=15 维相对特征<br/>已 StandardScaler"]
    F1 --> X1["x ∈ ℝ^{B × 10 × 15}"]
  end
...
| **特征数 F** | 15 维相对量（窗口以「日」计：1/5/14/20…） |
| **序列长度 T** | lookback = **10** 根日 K |
| **标签** | `close[t+1] > close[t]` → 1=涨, 0=跌 |
| **输出** | `P_day`、`direction∈{up,down}`；`as_of` = 最近交易日 |

### 2.2 网络拓扑（T=10 · F=15 · 2×64）

```mermaid
flowchart TB
  subgraph IN ["底层输入 (B, T=10, F=15)"]
    direction LR
    I0["bar₋₉<br/>15"] --- I1["bar₋₈<br/>15"] --- I2["…"] --- I8["bar₋₁<br/>15"] --- I9["bar₀<br/>15 ★最新"]
  end
...
flowchart TB
  ...
  subgraph seq [序列 · T=12 · F=15]
    WB --> WIN["最近 12 根已结束周 K"]
    WIN --> XF["每周 15 维特征<br/>窗口以「周」计"]
    XF --> XW["x ∈ ℝ^{B × 12 × 15}"]
  end
...
| **特征数 F** | 15（与日同构，滚动窗单位是 **周**） |
| **序列长度 T** | lookback = **12** 根周 K（约一个季度） |
| **标签** | 下一自然周收盘 > 本周收盘 → 涨/跌 |
| **输出** | `P_week`、`direction`；`as_of` = 最近已结束周的末日 |

### 3.2 网络拓扑（T=12 · F=15 · 2×64）

与日模型 **层结构同构**，仅序列更长（12 而非 10）：

```mermaid
flowchart TB
  subgraph IN ["底层输入 (B, T=12, F=15)"]
    direction LR
    I0["周₋₁₁<br/>15"] --- I1["周₋₁₀<br/>15"] --- I2["…"] --- I10["周₋₁<br/>15"] --- I11["周₀<br/>15 ★最新"]
  end
...
flowchart TB
  ...
  subgraph seq [序列 · T=6 · F=15]
    MB --> WIN["最近 6 根已结束月 K"]
    WIN --> XF["每月 15 维特征<br/>短窗: 1/3/6…"]
    XF --> XM["x ∈ ℝ^{B × 6 × 15}"]
  end
...
| **特征数 F** | 仍 **15**，窗口缩短（RSI=6 月、SMA=3/6 月…） |
| **序列长度 T** | lookback = **6** 根月 K |
| **标签** | 下一自然月收盘 > 本月收盘 → 涨/跌 |
| **输出** | `P_month`、`direction`；`as_of` = 最近已结束月的末日 |
| **容量** | `H=32`, `num_layers=1`, `dropout=0.4`（防过拟合） |

### 4.2 网络拓扑（T=6 · F=15 · 1×32）

单层 LSTM：无「层间 dropout」，隐含层只有一层；时间仍从左到右，分类头仍在顶上。

```mermaid
flowchart TB
  subgraph IN ["底层输入 (B, T=6, F=15)"]
    direction LR
    I0["月₋₅<br/>15"] --- I1["月₋₄<br/>15"] --- I2["月₋₃<br/>15"] --- I3["月₋₂<br/>15"] --- I4["月₋₁<br/>15"] --- I5["月₀<br/>15 ★最新"]
  end
...
```mermaid
flowchart TB
  subgraph dayBlock [日 · T=10 F=15]
    direction TB
    DI["输入 (B,10,15)"] --> DN["隐含: LSTM×2 H=64"] --> DO["分类: P_day → 涨/跌"]
  end

  subgraph weekBlock [周 · T=12 F=15]
    direction TB
    WI["输入 (B,12,15)"] --> WN["隐含: LSTM×2 H=64"] --> WO["分类: P_week → 涨/跌"]
  end

  subgraph monthBlock [月 · T=6 F=15]
    direction TB
    MI["输入 (B,6,15)"] --> MN["隐含: LSTM×1 H=32"] --> MO["分类: P_month → 涨/跌"]
  end
```
...
15 维相对特征（无绝对价）即网络输入的 **F=15**（含大盘相对）：

`return_1`, `return_long`, `index_return_1`, `index_return_long`, `excess_return_1`, `excess_return_long`, `price_sma_fast_dev`, `price_sma_slow_dev`, `sma_fast_slope`, `sma_slow_slope`, `RSI`, `macd_hist_norm`, `bb_pct_b`, `vol`, `volume_sma_ratio`

大盘基准（与个股同尺度聚合后按交易日 / 自然周 / 自然月对齐）：

| 市场 | 基准 |
|------|------|
| `cn_sh` / `cn_sz` | 沪深300（`sh000300`） |
| `cn_hk` | 恒生指数（`hkHSI`） |
| `us` | SPY |

其中 `excess_* = stock_return - index_return`。

```mermaid
flowchart LR
  OHLCV[个股 OHLCV] --> ALIGN[attach_benchmark_close]
  IDX[大盘 OHLCV] --> ALIGN
  ALIGN --> FE[build_features]
  FE --> Feat["特征矩阵 · F=15"]
  FE --> Y["target = 1{下一根收盘 > 本根}<br/>1=涨 0=跌"]
  Feat --> SEQ["滑动窗口 lookback=T<br/>末步 = 最新已完成 bar"]
  Y --> SEQ
  SEQ --> X["X:(N,T,15)  y:(N,) → StockLSTM"]
```

- 日/周特征窗：短/长收益 1&5，SMA 5&20，RSI 14，MACD 12/26/9，vol 20  
- 月特征窗：1&3，SMA 3/6，RSI 6，MACD 6/12/4，vol 6  

配置见 [`timeframes.py`](../backend/app/ml/timeframes.py)；基准映射见 [`stock_data.resolve_benchmark`](../backend/app/stock_data.py)。
...
3. `v6-scale` 与旧版 `v5-scale` 特征维不兼容，旧模型需重新训练。  
4. 验证集参与 early stopping，无独立 test。  
5. 仅供学习，不构成投资建议。
