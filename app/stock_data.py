...
def _load_cn_tickers() -> pd.DataFrame:
    try:
        return pd.read_csv("app/Data/cn_tickers.csv", dtype={"Symbol": str})
    except Exception:
...
def fetch_history(resolved: ResolvedTicker, bars: int = 180) -> pd.DataFrame:
    if is_chinese_market(resolved.market):
        return fetch_tencent_daily(resolved.tencent_code, bars=bars)
    return fetch_us_daily(resolved.display)
